...
python3 - <<'PY'
import json,sys,re,io,os,glob,subprocess,xml.dom.minidom
bad=[]
def check(c,m):
...
# A generated binary in the diff. One reached 0.5.1 because the tests import
# bin/nullius as a module and `git add -A` does not know the difference.
tracked=subprocess.run(["git","ls-files"],capture_output=True,text=True).stdout.split()
for f in tracked:
    check(not f.endswith(".pyc") and "__pycache__" not in f,
          f"{f}: generated bytecode is tracked. `git rm -r --cached` it; .gitignore has it now.")
